...
func syntaxDoc(theme, definition string) string {
	return "import tui 1.0\nimport demo 1.0\nimport demo.theme." + theme + " 1.0\n" +
		...
		"  theme.keyword: Theme.syntax.keyword; theme.string: Theme.syntax.string } }"
}
...
func TestS6WhatASyntaxHighlighterRefuses(t *testing.T) {
	for doc, want := range map[string]string{
		...
		`Editor { SyntaxHighlighter { theme.keywords: "red" } }`:   "theme.keywords",
		`Editor { SyntaxHighlighter { theme.keyword: "rouge" } }`:  "theme.keyword",
	} {
...
func TestS8AProgramsOwnHighlighterNeedsOnlyARegistration(t *testing.T) {
	...
	s := decltest.Run(t, 30, 3,
		tuidecl.LayoutSource("main.qml", []byte("import tui 1.0\nEditor { text: \"quiet LOUD\"\n SyntaxHighlighter { definition: \"Shout\"; theme.keyword: \"red\" } }")),
		tuidecl.Highlighters(map[string]highlight.Highlighter{"Shout": shout}))
