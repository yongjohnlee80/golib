...
>
> **A1-C4 — Failure ordering and poison.** A context cancellation or transport failure during send, read, or the post-emitter-error drain **poisons** the handle per §2.3 and the caller must `Discard`; the returned error is the transport error, which **outranks** a prior emitter error (the emitter's error is not what the wire's state means). A `SimpleQuery` on a non-quiescent segment state returns `ErrSegmentInFlight` immediately; with the session transaction OPEN it succeeds and the transaction remains open (the F1 case); **Transaction control is FORBIDDEN on this face, and the seam detects it without parsing SQL.** The session transaction has exactly one owner — `pinnedTx`, via `BeginSessionTx`/`CommitContext`/`RollbackContext` — and a `BEGIN`/`COMMIT`/`ROLLBACK`/`SAVEPOINT` arriving as `SimpleQuery` text would split the server's transaction state from `txOpen` and `pinnedTx.closed` (lector r2 MF5: a raw `COMMIT` clears `txOpen` while the `pinnedTx` object stays usable, so later operations run outside the transaction and its finalizer misreports success; a raw `BEGIN` is the inverse, an ownerless transaction). Two layers, and the second is the structural one: (i) autodb's pre-dispatch gate routes `ClassControl` statements through the `pinnedTx` lifecycle and never to `SimpleQuery` (A1-C1's bypass mutation covers a dispatch that forgets this); (ii) golib compares the terminal `ReadyForQuery` status byte against the transaction track it held before dispatch — **any transition between no-transaction (`I`) and in-transaction (`T`/`E`) in either direction** means the text contained transaction control that bypassed the owner, and the handle **poisons** and returns the typed `ErrTransactionControlOnRawFace` (the split state is not provably safe to reuse; `Discard` reclaims). The one legitimate change the text can cause, `T`→`E` (a statement failed inside the owned transaction), is reported to the owner: `pinnedTx` observes the aborted phase through the shared state, exactly as it does for the extended face. Cells: `SimpleQuery("BEGIN")` from idle → status `T` observed → poisoned + typed error; `SimpleQuery("COMMIT")` inside a live `pinnedTx` → poisoned + typed error, and the `pinnedTx`'s own `CommitContext` then reports the poisoned handle rather than success; a failing statement inside a live `pinnedTx` → `E`, not poison, and `RollbackContext` still works. Mutations: swallowing the transport error behind the emitter error; treating an `ErrorResponse` as poison; allowing dispatch mid-segment; dropping the status-transition check (the raw `BEGIN`/`COMMIT` cells redden).
