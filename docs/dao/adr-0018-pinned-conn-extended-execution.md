...
>
> - **(i) The complete boundary is autodb's pre-dispatch gate.** `ClassControl` statements — every spelling, including `SET TRANSACTION`, whose tag is the indistinguishable `SET` — are routed through the `pinnedTx` lifecycle and never reach `SimpleQuery`. Multi-statement text is gated per statement before the whole buffer is dispatched (A1-C1). This is the only layer that can see the SQL, so it is the only layer that can be complete. A1-C1's bypass mutation covers a dispatch that forgets it.
