...
// the field, or clear it (ADR-0010). The zero Rule is Skip — a rules map
...
// staged a value for it (ADR-0010 §2.3).
func Skip() Rule { return Rule{} }

// Clear returns a Rule that requests the field's cleared state. What that
// means is decided by the Field's clearability declaration (ADR-0010 §2.2):
