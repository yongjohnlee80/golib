...
func TestSearchOps(t *testing.T) {
	...
	tests := []struct {
		...
		{"string case-insensitive", StringOp("name", "artist.name"), "liq", "LOWER(artist.name) LIKE LOWER($1)", []any{"%liq%"}},
...
func TestSearchOp_FieldBinding(t *testing.T) {
	...
	if sql, _ := render(op.Predicate("q")); sql != "LOWER(namefield) LIKE LOWER($1)" {
		t.Errorf("unbound column = %q", sql)
	}
	// The schema binds the resolved column.
	bound := fb.withColumn("artist.name")
	if sql, _ := render(bound.Predicate("q")); sql != "LOWER(artist.name) LIKE LOWER($1)" {
